...
/*** Global Objects ***/
function topic(topicId, topicName, topicType, isEditable) {
			...
			this.renderHTML = function() {
				...
				var htmlContent = "<div class='col-lg-4 col-xs-8'>"
						+ "<!-- small box -->"
				...
				htmlContent = htmlContent + "<div class='inner'>" + "<h3 id="
						+ this.topicId
						+ " class='topic' style='cursor:pointer'><font size="
						+ this.fontSize() + ">" + this.topicName
						+ "</font></h3>" + "</div>" + "</div>"
						+ "</div><!-- ./col -->";
				return htmlContent;
			}
...
function QuestionAccordion(topicName, topicId, question_id,
		question_description, answer_explanation, additional_reference,
		isEditable) {
			this.topicName = topicName,
			this.topicId = topicId,
			...
			this.setTopicName = function() {
				var topicNameForQuestionsF = document
						.getElementById("topicNameForQuestions");
				if (topicNameForQuestionsF) {
					topicNameForQuestionsF.innerHTML = "<b>" + topicName
							+ "</b>";
				}
			},
			...
			this.buildQuestionContent = function() {
				...
				var accordionQuestionDivId = "accordionQuestionDiv"
						+ this.question_id;
				var accordionQuestionId = "accordionQuestion"
						+ this.question_id;
				var collapseHref = "collapse" + this.question_id;
				var accordionAnswerId = "accordionAnswer" + this.question_id;
				var accordionReferencesId = "accordionReferences"
						+ this.question_id;
				var accordionContent = "<div id='"
						+ accordionQuestionDivId
						...
						+ collapseHref + "'>" + this.question_description
						+ "</a>" + "</h4>";
				if (isEditable == true) {
				...
				accordionContent = accordionContent + "</div>" + "<div id='"
						+ collapseHref + "' class='panel-collapse collapse'>"
						+ "<div class='box-body'>" + "<div id='"
						+ accordionAnswerId + "'>" + this.answer_explanation
						+ "</div>" + "<div id='" + accordionReferencesId + "'>"
						+ "<p><u><i>References</i></u></p>"
						+ this.additional_reference + "</div>" +
...
function question(topicName, topicId, question_id, question_description,
		answer_explanation, additional_reference) {
	this.topicName = topicName, this.topicId = topicId,
			this.question_id = question_id,
			this.question_description = question_description,
			this.answer_explanation = answer_explanation,
			this.additional_reference = additional_reference,
...
function onWindowLoad(pathName) {
	...
	if (pathName.indexOf("/prepareForInterview/admin/questions/") > -1) {
		isEditable = true;
		topicIdSelected = pathName.replace("/prepareForInterview/admin/questions/", "");
		currentTopicIdSelected = topicIdSelected;
		onQuestionsLoad(topicIdSelected, isEditable);
		
	} else if (pathName.indexOf("/prepareForInterview/questions/") > -1){
		isEditable = false;
		topicIdSelected = pathName.replace("/prepareForInterview/questions/", "");
		currentTopicIdSelected = topicIdSelected;
		onQuestionsLoad(topicIdSelected, isEditable);
	}
	if (pathName == "/prepareForInterview/home" || pathName == "/prepareForInterview/admin/home") {
		if (pathName == "/prepareForInterview/admin/home"){
			isEditable = true;
		}
		getAllMainTopics(function(topics) {
			currentParentTopicIdSelected = "home";
			currentTopicIdSelected = "home";
			showTopics(topics,pathName);
			hide("addQuestionIdLink");
		});
	}

	if (pathName.indexOf("/prepareForInterview/admin/topic/") > -1) {
		isEditable = true;
		topicIdSelected = pathName.replace("/prepareForInterview/admin/topic/", "");
			...
			if (topic && topic.topic_id == topicIdSelected) {
				...
				getAllSubTopicsByTopicId(topicIdSelected, function(subTopics) {
					if (subTopics && subTopics.length > 0) {
						...
						getQuestionsByTopicId(topicIdSelected, function(
								questions) {
							if (questions && questions.length > 0) {
								window.location.pathname = "/prepareForInterview/admin/questions/"
										+ topicIdSelected;
							}
						});
				...
				window.location.pathname = "/prepareForInterview/admin/home";
			}
		});
	} else if (pathName.indexOf("/prepareForInterview/topic/") > -1) {
		isEditable = false;
				...
				getAllSubTopicsByTopicId(topicIdSelected, function(subTopics) {
					if (subTopics && subTopics.length > 0) {
						...
						getQuestionsByTopicId(topicIdSelected, function(
								questions) {
							if (questions && questions.length > 0) {
								window.location.pathname = "/prepareForInterview/questions/"
										+ topicIdSelected;
							}
						});
						//show("addQuestionIdLink");
		...
		getAllSearchResults(query, function(questionsPayload) {
			...
			if (questionsPayload && questionsPayload.length > 0) {
				for (var count = 0; count < questionsPayload.length; count++) {
					...
					var questionObject = new QuestionAccordion(
							firstQuestion.topic.topic_name,
							firstQuestion.topic.topic_id,
							firstQuestion.question_id,
							firstQuestion.question_description,
							firstQuestion.answer_explanation,
							firstQuestion.additional_reference, false);
	...
	updateBreadCrumbLinks(topicIdSelected, isEditable);
}

function onQuestionsLoad(topicIdSelected, isEditable) {
		...
		if (questionsPayload && questionsPayload.length > 0) {
			for (var count = 0; count < questionsPayload.length; count++) {
				...
				var questionObject = new QuestionAccordion(
						...
						firstQuestion.question_description,
						firstQuestion.answer_explanation,
						firstQuestion.additional_reference, isEditable);
				currentQuestionIdSelected = firstQuestion.question_id;
				questionObject.renderHTML();
				show("questionDiv");
				hide("addTopicId");
			...
			if(isEditable){
				window.location.href = "/prepareForInterview/admin/topic/" + topicIdSelected;
			} else {
...
function updateBreadCrumbLinks(topicIdSelected,isEditable) {
	//update bread crumb
	var breadCrumbElement = document.getElementById("topicsBreadcrumb");
	if (breadCrumbElement == null)
		return;
	if(isEditable){
		breadCrumbElement.innerHTML = "<li><a href='/prepareForInterview/admin/home'>Home</a></li>";
	} else {
	...
	getBreadcrumbLink(
			topicIdSelected,
			function(parentTopicsLink) {
				if (parentTopicsLink && parentTopicsLink.status == "success") {
					var topicIdLinks = parentTopicsLink.topicIdLink
							.split("-->");
					var topicNameLinks = parentTopicsLink.topicNameLink
							.split("-->");
					for (var i = 0; i < topicIdLinks.length - 1; i++) {
						breadCrumbElement.innerHTML += "<li><a id='"
								+ topicIdLinks[i]
								+ "breadCrumbId' class='breadCrumbLink' style='cursor:pointer'>"
								+ topicNameLinks[i] + "</a></li>";
					}
					breadCrumbElement.innerHTML += "<li class='active'>"
							+ topicNameLinks[topicNameLinks.length - 1]
							+ "</li>";
				}
			});
}
...
function showTopics() {
	...
	if (currentParentTopicIdSelected == "home") {
		...
		getAllSubTopicsByTopicId(currentParentTopicIdSelected, function(
				subTopics) {
			if (subTopics && subTopics.length > 0) {
				updateTopicsHTML(subTopics);
				hide("addQuestionIdLink");
...
function updateTopicsHTML(topics) {
	...
	for (var i = 0; i < topics.length; i++) {
		...
		var topicObject = new window.topic(topicJSON.topic_id,
				topicJSON.topic_name, topicJSON.topic_type,isEditable);
...
function togglePreviousNext(currentQuestionCount, questionsForCurrentTopic) {
	...
	if (questionsForCurrentTopic.length == 1
			|| questionsForCurrentTopic.length == 0) {
		$('#previousButtonId').prop('disabled', true);
		$('#nextButtonId').prop('disabled', true);
	}
	...
	if (currentQuestionCount == 0 && questionsForCurrentTopic.length == 1) {
		...
		var currentQuestionToBeDisplayed = new question(
				currentQuestion.topic.topic_name,
				currentQuestion.topic.topic_id, currentQuestion.question_id,
				currentQuestion.question_description,
				currentQuestion.answer_explanation,
				currentQuestion.additional_reference);
...
function getAllMainTopics(callback) {
	var promise = $.ajax({
		cache : false,
		type : "GET",
		url : '/prepareForInterview/rest/topics/listAllMainTopics'
	});
...
function getAllSubTopicsByTopicId(topicId, callback) {
	var promise = $.ajax({
		cache : false,
		type : "GET",
		url : '/prepareForInterview/rest/topics/listSubTopics/' + topicId
	});
...
function getBreadcrumbLink(topicId, callback) {
	var promise = $.ajax({
		cache : false,
		type : "GET",
		url : '/prepareForInterview/rest/topics/parentTopicsLink/' + topicId
	});
...
function getParentTopic(topicId, callback) {
	var promise = $.ajax({
		cache : false,
		type : "GET",
		url : '/prepareForInterview/rest/topics/parentTopic/' + topicId
	});
...
function getTopicByTopicId(topicId, callback) {
	var promise = $.ajax({
		cache : false,
		type : "GET",
		url : '/prepareForInterview/rest/topics/getTopic/' + topicId
	});
...
function deleteTopicByTopicId(topicId, callback) {
	var promise = $.ajax({
		cache : false,
		type : "DELETE",
		url : '/prepareForInterview/rest/topics/deleteTopic/' + topicId
	});
...
function checkIfTopicContainsSubTopicsOrQuestions(topicId, callback) {
	var promise = $.ajax({
		cache : false,
		type : "GET",
		url : '/prepareForInterview/rest/topics/checkTopicBeforeDelete/' + topicId
	});
...
function saveOrUpdateTopic(topicToBeSaved, callback) {
	var promise = $.ajax({
		...
		url : '/prepareForInterview/rest/topics/addOrEditTopic'
	});
...
$(function() {
	...
	$(document.body)
			.on(
					...
					function(e) {
						...
						getTopicByTopicId(topicIdForUpdate, function(
								topicForUpdate) {
							$('#topicIdTopicModalForm').val(
									topicForUpdate.topic_id);
							$('#topicNameTopicModalForm').val(
									topicForUpdate.topic_name);
	...
	$(document.body)
			.on(
					...
					function(e) {
						...
						checkIfTopicContainsSubTopicsOrQuestions(
								topicIdForDelete,
								function(response) {
															...
															function(result) {
																if (result.success == "true") {
																	...
																	$(
																			'#dialogMessageId')
																			.text(
																					"Couldn't delete the topic! Please try again!");
																}
																$(
																		'#dialogModal')
																		.modal(
	...
	$(document.body)
			.on(
					...
					function(e) {
						...
						var topicToBeSaved = {};
						topicToBeSaved.topic_id = $('#topicIdTopicModalForm')
								.val();
						topicToBeSaved.topic_name = $(
								'#topicNameTopicModalForm').val();
						...
						saveOrUpdateTopic(
								...
								function(result) {
									if (result.success == "true") {
										...
										$('#dialogMessageId')
												.text(
														"Topic has been added or updated successfully");
										$('#dialogModal').modal('show');
										if (window.location.pathname
												.indexOf("/prepareForInterview/questions") > -1) {
											window.location.pathname = "/prepareForInterview/topic/"
													+ topicToBeSaved.parent_topic_id;
										} else {
											showTopics();
										}
										...
										$('#dialogMessageId')
												.text(
														"Couldn't add or update the topic! Please try again!");
										$('#dialogModal').modal('show');
									}
	...
	$(document.body).on(
			...
			function() {
				...
				$('#topicIdTopicModalForm').val("");
				$('#topicIdTopicModalFormHelp').text(
				...
				if (currentParentTopicIdSelected == "home") {
					...
					getTopicByTopicId(currentTopicIdSelected, function(topic) {
						$('#topicTitleId').text(
								"Add new sub Topic in " + topic.topic_name);
					});
				}
	...
	$(document.body).on(
			'click',
			'a#addQuestionIdLink, #addQuestionIdLeft, #addQuestionIdRight',
			function() {
				getTopicByTopicId(currentTopicIdSelected, function(topic) {
					$('#questionTitleId').text(
							"Add new Question in " + topic.topic_name);
	...
	$(document.body)
			.on(
					'click',
					'#editQuestionAccordionId',
					function(evt) {

						currentQuestionIdSelected = evt.target.parentElement.parentElement.previousSibling.firstChild.id
								.replace("accordionQuestion", "");
						for (var q = 0; q < questionsForCurrentTopic.length; q++) {
							var questionObjectSelected = questionsForCurrentTopic[q];
							if (questionObjectSelected
									&& questionObjectSelected.question_id == currentQuestionIdSelected) {
								CKEDITOR.instances['questionIdTextArea']
										.setData(questionObjectSelected.question_description);
								CKEDITOR.instances['answerIdTextArea']
										.setData(questionObjectSelected.answer_explanation);
								CKEDITOR.instances['referencesIdTextArea']
										.setData(questionObjectSelected.additional_reference);
								//$('#difficultyLevelIdSelect').text(questionObjectSelected.importance);
							}
						}
	...
	$(document.body)
			.on(
					'click',
					'#deleteQuestionAccordionId',
					function(evt) {

						currentQuestionIdSelected = evt.target.parentElement.parentElement.previousSibling.firstChild.id
								.replace("accordionQuestion", "");
						deleteQuestionByQuestionId(
								currentQuestionIdSelected,
								function(result) {
									if (result.success == "true") {
										var div = document
												.getElementById("accordionQuestionDiv"
														+ currentQuestionIdSelected);
										if (div) {
										...
										$('#dialogMessageId')
												.text(
														"Couldn't delete the Question! Please try again!");
									}
									$('#dialogModal').modal('show');
	...
	$(document.body)
			.on(
					'click',
					'#btnQuestionSubmit, #btnQuestionSubmitHeader, #btnQuestionSubmitFooter',
					function() {
						...
						questionObject.question_description = CKEDITOR.instances['questionIdTextArea']
								.getData();
						questionObject.answer_explanation = CKEDITOR.instances['answerIdTextArea']
								.getData();
						questionObject.additional_reference = CKEDITOR.instances['referencesIdTextArea']
								.getData();
									...
									if (response.success == "true") {
										...
										if (questionsForCurrentTopic
											...
											if (questionStatus == "added") {
												questionsForCurrentTopic
														.push(questionAddedUpdated);
												var questionAccordion = new QuestionAccordion(
														questionAddedUpdated.topic.topic_name,
														questionAddedUpdated.topic.topic_id,
														questionAddedUpdated.question_id,
														questionAddedUpdated.question_description,
														questionAddedUpdated.answer_explanation,
														questionAddedUpdated.additional_reference,
														true);
												questionAccordion.renderHTML();
											} else if (questionStatus == "updated") {
												window.location.href = "/prepareForInterview/admin/questions/"
														+ questionAddedUpdated.topic.topic_id;
											}
										} else {
											window.location.href = "/prepareForInterview/admin/questions/"
										...
										$('#dialogMessageId')
												.text(
														"Question added or updated successfully!!!");
										$('#questionModal').modal('hide');
										$('#dialogModal').modal('show');
									} else {
										console
												.log("Error occured: Question not saved or updated!");
									}
								});
	...
	$(document.body)
			.on(
					"click",
					".topic, .breadCrumbLink",
					function(e) {

						var topicId = this.id;
						var isEditable = false;
						if(window.location.pathname.indexOf("/prepareForInterview/admin/") > -1){
							isEditable = true;
						}
						...
						if (window.location.pathname.indexOf("/prepareForInterview/admin/questions/") > -1) {
							window.location.pathname = "/prepareForInterview/admin/topic/" + topicId;
							return;
						}
						...
						updateBreadCrumbLinks(currentTopicIdSelected,isEditable);
										...
										getQuestionsByTopicId(
												...
												function(questionsPayload) {
													if (questionsPayload.length == 0) {
														var topicsDivId = document
																.getElementById("topicsDiv");
														topicsDivId.innerHTML = "";
														if (isEditable) {
															show("addQuestionIdLink");
														} else {
														...
														$('#dialogMessageId')
																.text(
																		"Neither Topics Nor Questions found for this topic");
														$('#dialogModal')
																.modal('show');
													} else {
														if(isEditable){
															...
															window.location.href = "/prepareForInterview/questions/"
																+ topicId;
														}
													}
												});
			...
			.on(
					...
					function(e) {
						...
						if (currentQuestionCount == 0) {
							...
							var currentQuestionToBeDisplayed = new question(
									currentQuestionObject.topic.topic_name,
									currentQuestionObject.topic.topic_id,
									currentQuestionObject.question_id,
									currentQuestionObject.question_description,
									currentQuestionObject.answer_explanation,
									currentQuestionObject.additional_reference);
							currentQuestionIdSelected = currentQuestionObject.question_id;
							currentQuestionToBeDisplayed.renderHTML();
							currentQuestionCount--;
						}

						togglePreviousNext(currentQuestionCount,
								questionsForCurrentTopic);
